...
class install {
  ...
  init() {
    ...
    steps
      ...
      .then(res => {
        this.inputs = {
          ...res,
          rootUrl: `http://localhost:${res.appPort}`,
          apiUrl: `http://localhost:${res.apiPort}/graphql`,
          uploadUrl: `http://localhost:${res.apiPort}/upload`,
        };
        this.performInstall().then(() => this.seed)();
      })
      .catch(e => {
  ...
  postInstall() {
    ...
    shell.exec(
      `rsync -a ./ ${this.inputs.installationPath}/ --exclude node_modules`
    );
