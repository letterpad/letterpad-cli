...
class install {
  ...
  init() {
    ...
    steps
      ...
      .then(res => {
        const baseName = "/" + res.baseName.replace(/^\/|\/$/g, "");
        let domain = res.domain.replace(/\/$/g, "") + baseName;
  ...
  postInstall() {
    ...
    copydir.sync(
      ...
      `${this.inputs.installationPath}/`,
      (stat, filename) => {
        if (stat === "directory" && filename.indexOf("node_modules") > 0) {
          return false;
        }
        return true;
      }
  ...
  finish() {
    const { rootUrl, apiUrl, uploadUrl, appPort, baseName } = this.inputs;
    envEditor.writeEnv(
