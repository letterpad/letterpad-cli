...
const common = {
  ...
  getDownloadObj: (releases, versionToInstall) => {
    let link = null;
    // check if the appropriate version is available
    for (var i = 0; i < releases.length; i++) {
      const { version } = releases[i];
      if (version === "v" + versionToInstall || version === versionToInstall) {
        link = releases[i];
        break;
      }
    }
    if (!link) {
      logger("No package found of version " + versionToInstall, logLevel.ERROR);
      process.exit();
    }
    return {
      zipball: link.zipball_url,
      version: link.zipball_url.substr(link.zipball_url.lastIndexOf("/") + 1),
    };
  },
